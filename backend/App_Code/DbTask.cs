        ...
        foreach (DataRow dr in dtParameters.Rows)
        {
            try
            {
                ...
                switch (para_type)
                {
                    ...
                    case DbType.Ntext:
                        {
                            oCommand.Parameters.Add("@" + para_name, SqlDbType.NText).Value = dr["para_value"].ToString();
                            break;
                        }
                    default:
                        {
...
//MultiDbTask
public class MultiDbTask
{
    private static IsolationLevel m_isoLevel = IsolationLevel.ReadUncommitted;
    private string strCon = string.Empty;
    public MultiDbTask()
    {
        strCon = Globals.ConectionString;
    }
    public MultiDbTask(string Server, string UserID, string Password)
    {
        strCon = "server = " + Server + "; uid = " + UserID + "; pwd = " + Password + "; charset = utf8;";
    }

    public MultiDbTask(string Server, string UserID, string Password, string Database)
    {
        strCon = "server = " + Server + "; uid = " + UserID + "; pwd = " + Password + "; Database = " + Database + "; charset = utf8;";
    }
